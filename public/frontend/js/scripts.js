...
function initSolonick() {
    ...
    fcshinit.on("init", function (event, slick) {
        ...
        TweenMax.to('.slick-active .half-hero-wrap h1', 1.5, {
            x: 0,
            opacity: 1,
            ease: Back.easeOut.config(1.7),
            delay: 1
        });
        TweenMax.to('.slick-active .half-hero-wrap h4', 1, {
            y: 0,
            opacity: 1,
            ease: Back.easeOut.config(1.7),
            delay: 1.5
    ...
    fcshinit.on("beforeChange", function (event, slick, currentSlide) {
        TweenMax.to('.slick-active .half-hero-wrap h1', 0, {x: 300, opacity: 0});
        TweenMax.to('.slick-active .half-hero-wrap h4', 0, {y: 50, opacity: 0});
    });
    fcshinit.on("afterChange", function (event, slick, currentSlide) {
        fssc.text(Number(slick.currentSlide + 1) + ' / ' + slick.slideCount);
        TweenMax.to('.slick-active .half-hero-wrap h1', 1.5, {x: 0, opacity: 1, ease: Back.easeOut.config(1.7)});
        TweenMax.to('.slick-active .half-hero-wrap h4', 1.5, {
            y: 0,
            opacity: 1,
    ...
    $('.sliding-menu a').on('click', function () {
        hideMenu();
        // return false;
    });
